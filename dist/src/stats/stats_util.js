 ...
 */
function getAgreeRate(voteTally, asProbabilityEstimate = true) {
...
function getTotalVoteCount(groupVoteTallies) {
    return Object.values(groupVoteTallies)
        .map((voteTally) => voteTally.totalCount)
        .reduce((a, b) => a + b, 0);
}
/**
 ...
 * @returns the actual or estimated agree probability
 */
function getTotalAgreeRate(voteInfo, asProbabilityEstimate = true) {
 ...
 */
function getTotalDisagreeRate(voteInfo, asProbabilityEstimate = true) {
    if ((0, types_1.isVoteTallyType)(voteInfo)) {
...
function getMinAgreeProb(comment, asProbabilityEstimate = true) {
    ...
    return Math.min(...Object.values(comment.voteInfo).map((voteTally) => getAgreeRate(voteTally, asProbabilityEstimate)));
}
/**
 ...
 * @returns the actual or estimated disagree probability
 */
function getDisagreeRate(voteTally, asProbabilityEstimate = true) {
    const totalCount = voteTally.totalCount;
    if (asProbabilityEstimate) {
 ...
 */
function getGroupInformedDisagreeConsensus(comment) {
    if ((0, types_1.isVoteTallyType)(comment.voteInfo)) {
        throw TypeError("Group information is required for calculating group informed disagree consensus.");
    }
    return Object.values(comment.voteInfo).reduce((product, voteTally) => product * getDisagreeRate(voteTally, true), 1);
}
/**
...
function getMinDisagreeProb(comment, asProbabilityEstimate = true) {
    ...
    return Math.min(...Object.values(comment.voteInfo).map((voteTally) => getDisagreeRate(voteTally, asProbabilityEstimate)));
}
/**
...
function getCommentVoteCount(comment) {
    ...
    if ((0, types_1.isVoteTallyType)(comment.voteInfo)) {
        ...
        return getTotalVoteCount(comment.voteInfo);
    }
}
