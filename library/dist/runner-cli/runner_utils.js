...
exports.concatTopics = concatTopics;
...
function getSummary(project, comments, topics, additionalContext) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    const summary = yield sensemaker.summarize(
      comments,
  ...
  for (const topic of comment.topics || []) {
    if ("subtopics" in topic) {
      for (const subtopic of topic.subtopics || []) {
        if ("subtopics" in subtopic && subtopic.subtopics.length) {
...
function parseTopicsString(topicsString) {
  ...
  const subtopicMappings = topicsString.split(";").reduce((topicMapping, topicString) => {
...
function getVoteInfoFromCsvRow(row, usesGroups, groupNames) {
  if (usesGroups) {
    ...
    for (const groupName of groupNames) {
      voteInfo[groupName] = new types_1.VoteTally(
    ...
    return new types_1.VoteTally(
      Number(row["agrees"]),
      Number(row["disagrees"]),
      Number(row["passes"])
    );
  }
}
