...
describe("Types Test", () => {
  it("The total votes should be the sum of all the different VoteTally values", () => {
    expect(new types_1.VoteTally(1, 2, 3).getTotalCount(true)).toEqual(6);
    expect(new types_1.VoteTally(1, 2).getTotalCount(true)).toEqual(3);
  });
  it("Valid VoteTallies should pass isVoteTallyType", () => {
    ...
    expect(
      (0, types_1.isVoteTallyType)({ agreeCount: 2, disagreeCount: 12, passCount: 0 })
    ).toBeTruthy();
    expect((0, types_1.isVoteTallyType)(new types_1.VoteTally(10, 5))).toBeTruthy();
  });
  ...
  it("Invalid CommentRecord should fail isCommentRecordType", () => {
    ...
    expect(
      (0, types_1.isCommentRecordType)({ id: 1, topics: [{ name: "Healthcare" }] })
    ).toBeFalsy();
    // Topics must be valid, the second one is missing a name.
    expect(
      (0, types_1.isCommentRecordType)({
        id: 1,
  ...
  it("Valid Topics should pass isTopicType", () => {
    ...
    expect(
      (0, types_1.isTopicType)({ name: "Test Topic", subtopics: [{ name: "Test Subtopic" }] })
    ).toBeTruthy();
  });
...
Claim 2 text. [id3] 
 Filler text`;
      ...
      it("should throw an error for unsupported format", () => {
        expect(() => summary.getText("UNSUPPORTED")).toThrow(
