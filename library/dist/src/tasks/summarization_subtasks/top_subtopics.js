...
class TopSubtopicsSummary extends recursive_summarization_1.RecursiveSummary {
    ...
    getSubtopicSummary(st, index) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const text = yield this.model.generateText((0, sensemaker_utils_1.getPrompt)(`Please generate a concise bulleted list identifying up to 5 prominent themes across all statements. Each theme should be less than 10 words long.  Do not use bold text. Do not preface the bulleted list with any text. These statements are all about ${st.name}`, subtopicComments.map((comment) => comment.text), this.additionalContext));
