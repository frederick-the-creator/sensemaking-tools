...
function categorizeWithRetry(model, instructions, inputComments, topics, additionalContext) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let attempts = 1; attempts <= model_util_1.MAX_RETRIES; attempts++) {
            ...
            const newProcessedComments = processCategorizedComments(newCategorized, inputComments, uncategorized, topics);
...
function processCategorizedComments(commentRecords, inputComments, uncategorized, topics) {
    ...
    const { commentsPassedValidation, commentsWithInvalidTopics } = validateCommentRecords(commentRecords, inputComments, topics);
...
function getTopicDepthFromTopics(topics, currentDepth = 1) {
    ...
    return topics.every((topic) => {
        ...
        ? getTopicDepthFromTopics(topics.map((topic) => ("subtopics" in topic ? topic.subtopics : [])).flat(), currentDepth + 1)
        : currentDepth;
}
/**
...
function getTopicsAtDepth(topics, depth) {
    if (depth === 1) {
        ...
        return getTopicsAtDepth(topics
            .map((topic) => {
...
function getCommentsWithTopic(comments, topicName) {
    return comments.filter((comment) => comment.topics && comment.topics.map((topic) => topic.name).includes(topicName));
}
/**
...
function getCommentTextsWithTopicsAtDepth(comments, topicName, depth = 1) {
    if (depth === 1) {
        ...
        return getCommentTextsWithTopicsAtDepth(comments
            .filter((comment) => {
    ...
    if ("subtopics" in topic) {
        ...
        for (let i = 0; i < topic.subtopics.length; i++) {
            if (topic.subtopics[i].name === parentSubtopic.name) {
                topic.subtopics[i] = addNewLevelToTopic(topic.subtopics[i], parentSubtopic.subtopics[0], newSubtopics);
            }
        }
        return topic;
    }
...
function mergeCommentTopics(comments, categorizedComments, topic, topicDepth) {
    const commentIdsInTopic = getCommentTextsWithTopicsAtDepth(comments, topic.name, topicDepth).map((comment) => comment.id);
    for (const commentId of commentIdsInTopic) {
        const matchingCategorized = categorizedComments.find((categorized) => categorized.id === commentId);
        ...
        for (let i = 0; i < comments.length; i++) {
            ...
            for (let j = 0; j < currentComment.topics.length; j++) {
                ...
                if (existingTopic.name === topic.name) {
                    currentComment.topics[j] = addNewLevelToTopic(existingTopic, topic, matchingCategorized.topics);
                }
 ...
 */
function categorizeCommentsRecursive(comments, topicDepth, model, topics, additionalContext, theme, factor, prompt_categorise_comments, prompt_learn_factors, prompt_learn_metrics, prompt_learn_themes) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!topics) {
            ...
            topics = yield (0, topic_modeling_1.learnOneLevelOfTopics)(comments, model, undefined, undefined, additionalContext, theme, factor, prompt_learn_factors, prompt_learn_metrics, prompt_learn_themes);
            let topicsCategorise = [];
            if (prompt_learn_themes) {
                topicsCategorise = topics.map((t) => ({ name: t.name }));
            }
            else {
                topicsCategorise = topics.flatMap((t) => "subtopics" in t && t.subtopics ? t.subtopics : []);
            }
            console.log("topics", topics);
            console.log("topicsCategorise", topicsCategorise);
            comments = yield oneLevelCategorization(comments, model, topicsCategorise, additionalContext, prompt_categorise_comments);
        ...
        for (let topic of parentTopics) {
            console.log("Categorizing statements into subtopics under: ", topic.name, ` (${++index}/${parentTopics.length} topics)`);
            const commentsInTopic = structuredClone(getCommentTextsWithTopicsAtDepth(comments, topic.name, currentTopicDepth));
            if (commentsInTopic.length === 0) {
                continue;
            }
            if (!("subtopics" in topic)) {
                // The subtopics are added to the existing topic, so a list of length one is returned.
                const newTopicAndSubtopics = (yield (0, topic_modeling_1.learnOneLevelOfTopics)(commentsInTopic, model, topic, parentTopics, additionalContext))[0];
            ...
            const categorizedComments = yield oneLevelCategorization(commentsInTopic, model, topic.subtopics, additionalContext);
...
function oneLevelCategorization(comments, model, topics, additionalContext, prompt_categorise_comments) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let i = 0; i < comments.length; i += model.categorizationBatchSize) {
            ...
            batchesToCategorize.push(() => categorizeWithRetry(model, instructions, uncategorizedBatch, topics, additionalContext));
        }
        ...
        // );
        const CategorizedBatches = yield (0, sensemaker_utils_1.executeConcurrently)(batchesToCategorize);
