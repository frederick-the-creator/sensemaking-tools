...
class SummaryStats {
    constructor(comments) {
        ...
        const topQuartilePassRate = get75thPercentile(this.filteredComments.map((comment) => (0, stats_util_1.getTotalPassRate)(comment.voteInfo, this.asProbabilityEstimate)));
    ...
    sortTopicStats(topicStats, sortByDescendingCount = true) {
        topicStats.sort((a, b) => {
            if (a.name === "Other")
                return sortByDescendingCount ? 1 : -1;
            if (b.name === "Other")
                return sortByDescendingCount ? -1 : 1;
