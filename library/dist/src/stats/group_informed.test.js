...
describe("GroupSummaryStats Test", () => {
  ...
  it("should count comments by topic", () => {
    ...
    expect(
      (_b = (_a = statsByTopic[0]) === null || _a === void 0 ? void 0 : _a.subtopicStats) ===
        null || _b === void 0
        ? void 0
        : _b.map((subtopic) => subtopic.commentCount)
    ).toEqual([2, 1]);
  });
  ...
  it("should get the representative comments for a given group", () => {
    const representativeComments = new group_informed_1.GroupedSummaryStats(
      TEST_COMMENTS
  ...
  it("should return empty array if there are no comments with vote tallies", () => {
    ...
    expect(
      new group_informed_1.GroupedSummaryStats(commentsWithoutVotes).getGroupRepresentativeComments(
        "0"
      )
    ).toEqual([]);
  });
