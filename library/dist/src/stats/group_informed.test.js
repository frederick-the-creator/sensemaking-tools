...
const group_informed_1 = require("./group_informed");
...
const TEST_COMMENTS = [
    {
        ...
        text: "comment1",
        voteInfo: {
            "0": new types_1.VoteTally(20, 10, 0),
...
describe("GroupSummaryStats Test", () => {
    it("should get the total number of votes from multiple comments", () => {
        const summaryStats = new group_informed_1.GroupedSummaryStats(TEST_COMMENTS);
    ...
    it("should count comments by topic", () => {
        ...
        const statsByTopic = new group_informed_1.GroupedSummaryStats(comments).getStatsByTopic();
        expect(statsByTopic[0].commentCount).toEqual(3);
        expect((_b = (_a = statsByTopic[0]) === null || _a === void 0 ? void 0 : _a.subtopicStats) === null || _b === void 0 ? void 0 : _b.map((subtopic) => subtopic.commentCount)).toEqual([
            2, 1,
        ]);
    });
    ...
    it("should get the representative comments for a given group", () => {
        const representativeComments = new group_informed_1.GroupedSummaryStats(TEST_COMMENTS).getGroupRepresentativeComments("0");
    ...
    it("should return empty array if there are no comments with vote tallies", () => {
        const commentsWithoutVotes = [
            { id: "1", text: "comment1" },
            { id: "2", text: "comment2" },
        ];
        expect(new group_informed_1.GroupedSummaryStats(commentsWithoutVotes).getGroupRepresentativeComments("0")).toEqual([]);
    });
});
